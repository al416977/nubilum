...
def list_files():
    ...
    files = [f.name for f in RAW_DIR.iterdir() if f.is_file() and f.suffix.lower() in allowed_ext]
    files.sort()
    return {"files": files}
...
def upload_file(file: UploadFile = File(...)):
    ...
    return {"message": "Archivo subido correctamente", "file_name": file.filename, "saved_to": str(save_path)}


@app.get("/preprocess")
def preprocess(file_name: str, voxel_size: float = 0.30, nb_neighbors: int = 16, std_ratio: float = 10.0):
    try:
        return run_preprocess(file_name=file_name, voxel_size=voxel_size, nb_neighbors=nb_neighbors, std_ratio=std_ratio)
...
def classify(project: str):
    try:
        ...
        return {"message": "Clasificación completada", "project": project, "report": report}
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
